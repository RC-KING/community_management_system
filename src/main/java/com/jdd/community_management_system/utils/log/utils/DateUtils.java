...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
  ...
  /**
   * 把毫秒转为 日期字符串 yyyy-MM-dd hh:mm:ss SSS a yyyy/MM/dd hh:mm:ss SSS a
   *
   * @param millisecond
   * @return
   */
  ...
  public static void main(String[] args) throws ParseException {
    ...
    long millisecond = new Date().getTime() + 86400000;
    Date date = new Date(millisecond);
    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss SSS");
    System.out.println("毫秒[1483159625851]对应日期时间字符串：" + format.format(date));
    ...
    System.out.println(formatDateTime(new Date().getTime() + 86400000));
  ...
  public static int getTimeDistance(Date beginDate, Date endDate) {
    ...
    endCalendar.add(
        Calendar.DAY_OF_MONTH, -betweenDays); // 使endCalendar减去这些天数，将问题转换为两时间的毫秒数之差不足一天的情况
    endCalendar.add(Calendar.DAY_OF_MONTH, -1); // 再使endCalendar减去1天
    if (beginCalendar.get(Calendar.DAY_OF_MONTH)
        == endCalendar.get(Calendar.DAY_OF_MONTH)) // 比较两日期的DAY_OF_MONTH是否相等
    return betweenDays + 1; // 相等说明确实跨天了
    else return betweenDays + 0; // 不相等说明确实未跨天
  }
}
